...
class GameState:
    ...
    def trackRepetition(self):
        """ Track the current board position in the repetition tracker. """
        fen = self.boardToFEN()
        if fen not in self.repetition_tracker:
            self.repetition_tracker[fen] = 0
        self.repetition_tracker[fen] += 1

    def isThreefoldRepetition(self):
        """ Check for threefold repetition. """
        fen = self.boardToFEN()
        return self.repetition_tracker.get(fen, 0) >= 3
    
    def boardToFEN(self):
        """
        Convert the current game board to FEN notation.
        """
        fen_rows = []
        ...
        # Placeholder for castling and en passant rights
        castling_rights = "-"  # Adjust this to reflect actual castling rights
        en_passant = "-"  # Adjust this to reflect actual en passant rights

        # Halfmove clock and fullmove number
        halfmove_clock = 0  # Adjust according to your move tracking
        fullmove_number = len(self.move_log) // 2 + 1
